...
function formatRelativeTime(
  ...
  const lastable =
    ['hours', 'minutes', 'seconds'].indexOf(unit) === -1;
...
class FixedOffsetZone extends Zone {
  ...
  equals(otherZone) {
    return (
      otherZone.type === 'fixed' && otherZone.fixed === this.fixed
    );
  }
...
function systemLocale() {
  if (sysLocaleCache) {
    ...
    sysLocaleCache = new Intl.DateTimeFormat().resolvedOptions()
      .locale;
    return sysLocaleCache;
  }
}
...
const offsetRegex = /(?:(Z)|([+-]\d\d)(?::?(\d\d))?)/,
  ...
  extractISOWeekData = simpleParse(
    'weekYear',
    'weekNumber',
    'weekDay'
  ),
...
function extractISODuration(match) {
  ...
  const maybeNegate = (num, force = false) =>
    num !== undefined && (force || (num && hasNegativePrefix))
      ? -num
      : num;
...
class Duration {
  ...
  constructor(config) {
    const accurate =
      config.conversionAccuracy === 'longterm' || false;
    /**
     * @access private
     */
    ...
    for (const k of orderedUnits$1) {
      if (units.indexOf(k) >= 0) {
        ...
        for (const down in vals) {
          if (
            orderedUnits$1.indexOf(down) > orderedUnits$1.indexOf(k)
          ) {
...
class Info {
  ...
  static hasDST(zone = Settings.defaultZone) {
    ...
    return (
      !zone.isUniversal &&
      proto.offset !== proto.set({ month: 6 }).offset
    );
  }
...
function fixListRegex(s) {
  ...
  return s
    .replace(/\./g, '\\.?')
    .replace(spaceOrNBSPRegExp, spaceOrNBSP);
}
...
function maybeExpandMacroToken(token, locale) {
  ...
  const tokens = parts.map((p) =>
    tokenForPart(p, locale, formatOpts)
  );
...
function quickDT(obj, opts) {
  ...
  if (!isUndefined(obj.year)) {
    ...
    const invalid =
      hasInvalidGregorianData(obj) || hasInvalidTimeData(obj);
    if (invalid) {
...
function diffRelative(start, end, opts) {
  ...
  return format(
    start > end ? -0 : 0,
    opts.units[opts.units.length - 1]
  );
}
...
function lastOpts(argList) {
  ...
  if (
    argList.length > 0 &&
    typeof argList[argList.length - 1] === 'object'
  ) {
...
class DateTime {
  ...
  setZone(
    zone,
    { keepLocalTime = false, keepCalendarTime = false } = {}
  ) {
